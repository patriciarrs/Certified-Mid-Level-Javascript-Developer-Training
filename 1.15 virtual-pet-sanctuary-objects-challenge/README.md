...
2. Add to Sanctuary:
   ...
   - Implement a function `addToSanctuary` that takes a virtual pet object (as a parameter) and adds it to the sanctuary. The pet's name should be the key, and the pet object should be the value.
   - Add the three pets created in step 1 to the sanctuary and log the resulting object.

3. Check Sanctuary:
   - Implement a function `isPetInSanctuary` that takes a pet name (as a parameter) and returns true if the pet is in the sanctuary, false otherwise. Use the `in` operator.
...
5. Find Happy Pets:
   ...
   - It should return an object's array of pet names that have a mood greater than or equal to the threshold.
