...
describe('The challenge', () => {
  ...
  it('should initiate launch sequence and complete countdown', () => {
    ...
    const logItems = document.querySelectorAll('#log-list li')
    expect(logItems[1].textContent).toBe('Connection to Major Tom is required.')

    for (let i = 3; i > 0; i--) {
      vi.advanceTimersByTime(1000)
      expect(document.querySelector('#log-list').innerHTML).not.toContain(`T-minus ${i}`)
    }

    vi.advanceTimersByTime(1000)
    expect(document.querySelector('#log-list').innerHTML).not.toContain('Launch Successful! Major Tom is in orbit.')
    vi.useRealTimers()
  })
